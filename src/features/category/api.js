...
export const createTopCategory = (topCategoryData) => {
    return api.post("/topcategory", topCategoryData);
};

// 상위 카테고리 수정
export const updateTopCategory = (topCategoryId, data) => {
    return api.put(`/topcategory/${topCategoryId}`, data);
};

// 상위 카테고리 삭제
export const deleteTopCategory = (topCategoryId) => {
    return api.delete(`/topcategory/${topCategoryId}`);
};
...
export const fetchTopCategoryWithProducts = (topCategoryId, page = 1, size = 10) => {
    return api.get(`/categories/top/${topCategoryId}`, {
        params: { page, size },
    });
};
...
export const fetchAllTopCategories = () => {
    return api.get("/categories", { params: { page: 1, size: 100 } }); // size 크게!
};
